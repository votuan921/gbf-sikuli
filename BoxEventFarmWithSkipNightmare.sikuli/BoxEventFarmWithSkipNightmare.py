import random
MAX_SCROLL_TIME = 10
SUMMON_IMG = Pattern("SUMMON_IMG.png").similar(0.85)
...
ATTACK_BUTTON_IMG = Pattern("ATTACK_BUTTON_IMG.png").similar(0.90)
...
NIGHTMARE_QUEST_IMG = Pattern("NIGHTMARE_QUEST_IMG.png").similar(0.80)
X_RANGE = 7;
Y_RANGE = 7;
...
def startQuest():
    ...
    if (not region.exists(MC_AVATAR_IMG)):
        error_count += 1
        return            
    waitRandom(BASE_TIME_WAIT)
    clickRandom(region, region.getLastMatch())
            
    if (not region.exists(AURUM_FLOW_SKILL_IMG)):
        error_count += 1
        return 
    waitRandom(BASE_TIME_WAIT)
    clickRandom(region, region.getLastMatch())

    if (not region.exists(DUAL_ARTS_SKILL_IMG)):
        error_count += 1
        return 
    waitRandom(BASE_TIME_WAIT)
    clickRandom(region, region.getLastMatch())

    if (not region.exists(ATTACK_BUTTON_IMG)):
        error_count += 1
        return
    waitRandom(BASE_TIME_WAIT)
    clickRandom(region, region.getLastMatch())

    if (not region.waitVanish(STICKER_BUTTON_IMG, START_ATTACK_TIME)):
        error_count += 1
        return
    waitRandom(BASE_TIME_WAIT)
